...
langs[3] = "javascript"                  # 끝에 추가
print(langs)

langs.remove("c++")                # 인덱스 2에 "c#" 추가
print(langs)
...
result = [x for x in range(scores) if x >= 60]
print(result)# ✅ [85, 73, 90, 68, 100] 출력


# 2️⃣ 60점 이상인 경우 "합격", 60점 미만은 "불합격"으로 처리
result = ["합격" if x >= 60 else "불합격" for x in range(scores)]
...
result = [word for word in range(words) if 'n' in word]  
...
result = [sum(scores[i])/3 for i in range(1, 4)]
